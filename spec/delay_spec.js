  ...
  datePairs.map(function (testCase) {
    it(testCase.name, function (done) {
      ...
      model.blacklist.delayOnce(function (err, data) {
        ...
        model.blacklist.list(function (err, data) {
          expect(err).toBeNull()
          expect(data).toEqual([testCase.output])
          done()
        })
      })
    })
...
describe('delay until for', function () {
  const datePairs = [
    ...
    { today: '2017-06-15 15:00:00', input: '2017-06-15', output: [], name: 'Thursday to itself after fika' },
  ...
  datePairs.map(function (testCase) {
    it(testCase.name, function (done) {
      ...
      model.blacklist.delayUntil(testCase.input, function (err, data) {
        expect(err).toBeNull()
        expect(data.dates.map(d => moment(d).format('YYYY-MM-DD'))).toEqual(testCase.output)
        model.blacklist.list(function (err, data) {
          expect(data).toEqual(testCase.output)
          done()
        })
